...
/**
 * This class is design to handle the chunks sent in a streaming
 * v4 Auth request
 */
export default class V4Transform extends Transform {
    ...
    _parseMetadata(remainingChunk) {
        ...
        if (Number.isNaN(dataSize)) {
            this.log.trace('chunk body did not contain size');
            return { err: errors.InvalidArgument };
        }
        ...
        const unparsedChunk = remainingChunk.slice(lineBreakIndex + 2);
        return { completeMetadata: true, unparsedChunk };
    }
